...
void SetMissileFireType(const int shipIndex, const int type, const bool at_self)
{
	switch(type)
	{
		case MISSILE_SINGLE:
		{
			Ships[shipIndex].flags &= ~SF_SECONDARY_DUAL_FIRE;
			if(at_self)
			{	
...
int button_function_critical(int n, net_player *p = NULL)
{
	...
	player *pl;
	net_player *npl;
	bool at_self;    // flag indicating the object is local (for hud messages, etc)
	...
	switch (n) {				
		...
		case CHOOSE_SINGLE_SECONDARY:
	

			if ( n == CHOOSE_SINGLE_SECONDARY) {		
				SetMissileFireType(objp->instance, MISSILE_SINGLE, at_self);
			} else {
				SetMissileFireType(objp->instance, MISSILE_DUAL, at_self);
			}
...
//				}					
		...
		case CHOOSE_PRIMARY_1_OFF:
		case CHOOSE_PRIMARY_2_OFF:
		case CHOOSE_PRIMARY_ALL_ON:
			ship_set_primary(Player_obj, n);
			break;
...
int button_info_query(button_info *bi, int n)
{
	...
	if(pPad && pPad->IsActive())
	{
		return pPad->GetInputControl(n);
	}
