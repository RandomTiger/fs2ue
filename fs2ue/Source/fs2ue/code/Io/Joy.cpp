...
void joy_close()
{
	...
	if(Joy_tell_thread_to_end_event)
	{
		SetEvent(Joy_tell_thread_to_end_event);
	}

	// Wait for it to end
	if (Joy_thread_says_its_done_event && WaitForSingleObject( Joy_thread_says_its_done_event, 5000 )==WAIT_TIMEOUT)	{			//INFINITE );
	...
	if(Joy_thread_says_its_done_event)
	{
