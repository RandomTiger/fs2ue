...
enum
{
	...
	CLONE_BUTTON,
	DELETE_BUTTON,
	SCROLL_LIST_UP_BUTTON,
	SCROLL_LIST_DOWN_BUTTON,
	ACCEPT_BUTTON,
	SINGLE_BUTTON,
	MULTI_BUTTON,
	NUM_PLAYER_SELECT_BUTTONS,
};
...
void player_select_close()
{
	...
	{
		PlayerCoOp = &Players[1];
		PlayerCoOp->flags |= PLAYER_FLAGS_STRUCTURE_IN_USE;

		char *pLoadName = "Tom";
		if(strcmp(Pilots[Player_select_pilot], pLoadName) == 0)
		{
			pLoadName = "Zosiast";
		}

		// Hack to load pilots I know exist
		if (read_pilot_file(pLoadName, !Player_select_mode, PlayerCoOp) != 0) {
			Error(LOCATION,"Couldn't load pilot file, bailing");
			PlayerCoOp = NULL;
		}
	}
