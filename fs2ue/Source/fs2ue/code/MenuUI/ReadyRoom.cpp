...
const int CAMPAIGN_MISSION_HASH_SIZE = 307;
...
const int NUM_SIM_MISC_TEXT		= 		2;
const int SIM_MISC_TEXT_MISSION	 = 		0;
const int SIM_MISC_TEXT_FILENAME =		1;
...
enum 
{	
	READYROOM_LINE_CAMPAIGN	= 1,
	READYROOM_LINE_CMISSION,
	READYROOM_LINE_MISSION,
};

const int READYROOM_FLAG_FROM_VOLITION	= (1<<0);			// volition made
static struct {	
...
int readyroom_continue_campaign()
{
	...
	// CD CHECK
	if(GlobalConst::kCDCheckOn && !game_do_cd_mission_check(Game_current_mission_filename)){		
		return -1;
	}
...
void sim_room_close()
{
	...
	for (i=0; i<Num_campaign_missions; i++)
		if (Campaign_missions[i])
			vm_strdup_free(Campaign_missions[i]);
	...
	if (Standalone_mission_names_inited){
		for (i=0; i<Num_standalone_missions; i++){
			if (Standalone_mission_names[i]){
				vm_strdup_free(Standalone_mission_names[i]);
			}
	...
	if (Campaign_names_inited)
		for (i=0; i<Num_campaigns; i++)
			if (Campaign_names[i])
				vm_strdup_free(Campaign_names[i]);

	if (Campaign_mission_names_inited)
		for (i=0; i<Campaign.num_missions; i++)
			if (Campaign_mission_names[i])
				vm_strdup_free(Campaign_mission_names[i]);

	for (i=0; i<Num_campaigns; i++)
		vm_strdup_free(Campaign_file_names[i]);

	for (i=0; i<Num_standalone_missions; i++)
		vm_strdup_free(Mission_filenames[i]);
