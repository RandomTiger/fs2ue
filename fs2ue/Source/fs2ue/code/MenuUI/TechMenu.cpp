...
const int MAX_TEXT_LINES = 100;
const int MAX_TEXT_LINE_LEN = 256;
...
void techroom_ships_render(float frametime)
{
	...
	{
		g_MouseController.GetDelta(&dx, &dy, NULL, NULL, 1);
		if(dx != 0 || dy != 0)
		{
			Trackball_active = 1;
		}
...
void techroom_do_frame(float frametime)
{
	...
	if (Trackball_active && g_MouseController.GetType() != InputController::kIN_360PAD) {
		Ui_window.set_ignore_gadgets(1);
	} else {
