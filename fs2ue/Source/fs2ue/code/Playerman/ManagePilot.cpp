...
int read_pilot_file(char *callsign, int single, player *p)
{
   ...
   Game_skill_level = cfread_int(file);
	...
	for (i=0; i<NUM_JOY_AXIS_ACTIONS; i++) {
		AxisMap::GetAxisMapTo(true)[i] = cfread_int(file);
	...
	g_MouseController.SetMouseState(cfread_int(file) ? MouseController::MOUSE_STATE_USE_TO_FLY : 0);
	g_MouseController.SetMouseSensitivity(cfread_int(file));
...
int write_pilot_file_core(player *p)
{
   ...
   cfwrite_int(Game_skill_level, file);
	...
	for (i=0; i<NUM_JOY_AXIS_ACTIONS; i++) {
		cfwrite_int(AxisMap::GetAxisMapTo(true)[i], file);
   ...
   cfwrite_int(Briefing_voice_enabled, file);
	...
	int Use_mouse_to_fly  = g_MouseController.GetMouseState(MouseController::MOUSE_STATE_USE_TO_FLY);
	int Mouse_sensitivity = g_MouseController.GetMouseSensitivity();
...
#error Code in ManagePilot assumes NUM_DEFAULT_DETAIL_LEVELS = 4
		...
		g_MouseController.SetMouseSensitivity(4);
		Joy_sensitivity = 9;
		Dead_zone_size = 10;
	}
...
void player_set_squad_bitmap(player *p, char *fname)
{
	...
	if(strlen(p->squad_filename) > 0){
		...
		if(p->insignia_texture != -1){
			bm_lock(p->insignia_texture, kDefaultBpp, BMP_TEX_XPARENT);
