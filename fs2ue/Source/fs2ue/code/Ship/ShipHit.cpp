...
void player_died_start(const object * const player_obj, object * killer_objp)
{
...
void ship_hit_kill(object *ship_obj, object *other_obj, float percent_killed, int self_destruct)
{
	...
	if (IsPlayerObj(ship_obj)) {
		player_died_start(ship_obj, other_obj);
	}
...
// player pain
void ship_hit_pain(object *ship_obj, float damage)
{
	MsgFlash lMsg(damage/15.0f, -damage/30.0f, -damage/30.0f);
	ship_obj->PostAMessage(&lMsg);
