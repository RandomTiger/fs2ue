...
int ship_ship_check_collision(collision_info_struct *ship_ship_hit_info, vector *hitpos)
{
	...
	player_involved = IsPlayerObj(heavy_obj) || IsPlayerObj(Player_obj);
...
void maybe_push_little_ship_from_fast_big_ship(object *lBig, object *lSmall, float impulse, vector *normal)
{
	// Move player out of the way of a BIG|HUGE ship warping in or out
	if (Ship_info[Ships[lBig->instance].ship_info_index].flags & (SIF_CAPITAL|SIF_SUPERCAP)) {
		if (Ship_info[Ships[lSmall->instance].ship_info_index].flags & (SIF_SMALL_SHIP)) {
			float big_speed = vm_vec_mag_quick(&lBig->phys_info.vel);
			if (big_speed > 3*lBig->phys_info.max_vel.z) {
				// push player away in direction perp to forward of big ship
				// get perp vec
				vector temp, perp;
				vm_vec_sub(&temp, &lSmall->pos, &lBig->pos);
				vm_vec_scale_add(&perp, &temp, &lBig->orient.fvec, -vm_vec_dotprod(&temp, &lBig->orient.fvec));
				...
				float added_perp_vel_mag = impulse / lSmall->phys_info.mass;

				// add to vel and ramp vel
				vm_vec_scale_add2(&lSmall->phys_info.vel, &perp, added_perp_vel_mag);
				vm_vec_rotate(&lSmall->phys_info.prev_ramp_vel, &lSmall->phys_info.vel, &lSmall->orient);
			}
		}
	}
}
...
//			vm_vec_add(&world_hit_pos, &ship_ship_hit_info.heavy->pos, &ship_ship_hit_info.hit_pos);
			...
			if (player_involved && (ship_ship_hit_info.impulse > 0)) {
				...
				g_ForceFeedback.PlayVectorEffect(&v, scaler);
			}
