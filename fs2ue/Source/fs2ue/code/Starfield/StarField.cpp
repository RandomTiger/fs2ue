...
bool last_stars_filled[kiNumPlayerSplitscreen];

typedef struct star {
	vector pos;
	vector last_star_pos[kiNumPlayerSplitscreen];
} star;
...
// old rectangular distribution scheme.
	...
	for(int i = 0; i < kiNumPlayerSplitscreen; i++)
	{
		last_stars_filled[i] = 0;
	}
...
{	
	...
	MsgSetSunDrawCount lMsg(lSunDrawCount);
...
void stars_draw( int show_stars, int show_suns, int show_nebulas, int show_subspace )
{
	...
	if(Game_detail_flags & DETAIL_FLAG_NEBULAS && Neb2_render_mode == NEB2_RENDER_SIMPLE)
	{
		color clearColor = gr_screen.current_clear_color;

		int r,b,g;
		neb2_get_backg_color(&r, &g, &b);
		gr_set_clear_color(r,g,b);
		gr_clear();

		gr_screen.current_clear_color = clearColor;
	}
	else if ( show_nebulas && (Game_detail_flags & DETAIL_FLAG_NEBULAS) && (Neb2_render_mode != NEB2_RENDER_POF) && (Neb2_render_mode != NEB2_RENDER_LAME))	{
		nebula_render();
	}

	// draw background stuff	
	if((Neb2_render_mode != NEB2_RENDER_POLY) && (Neb2_render_mode != NEB2_RENDER_LAME) && (Neb2_render_mode != NEB2_RENDER_SIMPLE) && show_stars){
	...
	if (show_stars && ( Game_detail_flags & DETAIL_FLAG_STARS) && !(The_mission.flags & MISSION_FLAG_FULLNEB) && (supernova_active() < 3))	{
		...
		if ( last_stars_filled[PlayerIndex] == false )	{
			for (sp=Stars,i=0; i<Num_stars; i++, sp++ ) {
				vertex p2;
				g3_rotate_faraway_vertex(&p2, &sp->pos);
				sp->last_star_pos[PlayerIndex].x = p2.x;
				sp->last_star_pos[PlayerIndex].y = p2.y;
				sp->last_star_pos[PlayerIndex].z = p2.z;
			}
		}
		...
		for (sp=Stars,i=0; i<tmp_num_stars; i++, sp++ ) {
			...
			if ( can_draw && (Star_flags & (STAR_FLAG_TAIL|STAR_FLAG_DIM)) )	{
				...
				dist = vm_vec_dist_quick( &sp->last_star_pos[PlayerIndex], (vector *)&p2.x );
				...
				p1.x = p2.x + (sp->last_star_pos[PlayerIndex].x-p2.x)*ratio;
				p1.y = p2.y + (sp->last_star_pos[PlayerIndex].y-p2.y)*ratio;
				p1.z = p2.z + (sp->last_star_pos[PlayerIndex].z-p2.z)*ratio;
	...
	last_stars_filled[PlayerIndex] = true;
