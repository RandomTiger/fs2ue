...
void training_obj_display()
{
	...
	// print out title
	emp_hud_printf(
		Directive_coords[gr_screen.res][DIRECTIVE_COORDS_TITLE][0]+fl2i(HUD_offset_x), 
		Directive_coords[gr_screen.res][DIRECTIVE_COORDS_TITLE][1]+fl2i(HUD_offset_y), 
		EG_OBJ_TITLE, 
		eAlignCentre,
		XSTR( "directives", 422));
...
//				gr_set_color_fast(&Color_bright_blue);
		...
		emp_hud_string(x, y, EG_OBJ1 + i, buf, eAlignCentre);
		// gr_printf(x, y, buf);
		...
		if ( second_line ) {
			...
			emp_hud_string(x+12, y, EG_OBJ1 + i + 1, second_line, eAlignCentre);
