...
public:
	...
	unsigned char GetByte(const int offset, const int fileOffset);
	ubyte			*cache;
...
class AnimInstanceGroup
{
public:
	AnimInstanceGroup() : m_anim(0) {}

	bool Load(const char * const filename);
	int GetBitmap(int frame);
	int GetNumFrames();
	void Unload();
private:
	std::vector<anim_instance *> m_instances;
	std::vector<int> m_bitmapIDs;
	anim *m_anim;
};
