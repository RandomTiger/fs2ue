...
bool IsViewModeForHUD()
{
	uint lFlagList = VM_SLEWED | VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY;
	lFlagList |= VM_CHASE;

	return !(Viewer_mode & lFlagList);
}
...
void hud_maybe_blit_head_border()
{
	...
	if ( message_anim_is_playing() ) {
		...
		HUD_AABITMAP(Head_frame_gauge.first_frame, Head_frame_coords[gr_screen.res][0], Head_frame_coords[gr_screen.res][1], eAlignLeft);		

		// draw title
		HUD_STRING(Head_message_coords[gr_screen.res][0], Head_message_coords[gr_screen.res][1], XSTR("message", 217), eAlignLeft);
	}
}
...
void hud_show_damage_popup()
{
	...
	HUD_AABITMAP(Damage_gauges[0].first_frame, Damage_gauge_coords[gr_screen.res][0][0], Damage_gauge_coords[gr_screen.res][0][1], eAlignCentre);	
	HUD_STRING(Damage_text_coords[gr_screen.res][0], Damage_text_coords[gr_screen.res][1], XSTR( "damage", 218), eAlignCentre);
	...
	if ( screen_integrity < 100 ) {		
		...
		HUD_STRING(Hull_integ_coords[gr_screen.res][0], Hull_integ_coords[gr_screen.res][1], XSTR( "Hull Integrity", 220), eAlignCentre);
		HUD_STRING(Hull_integ_val_coords[gr_screen.res][0] - w, Hull_integ_val_coords[gr_screen.res][1], buf, eAlignCentre);
	} 
	...
	for ( int i = 0; i < num; i++ ) {
		...
		HUD_AABITMAP(Damage_gauges[1].first_frame, bx, by, eAlignCentre);
		...
		HUD_STRING( sx, sy, hud_targetbox_truncate_subsys_name(hud_subsys_list[best_index].name), eAlignCentre);
		sprintf(buf, XSTR( "%d%%", 219), best_str);
		hud_num_make_mono(buf);
		gr_get_string_size(&w, &h, buf);
		HUD_STRING(Hull_integ_val_coords[gr_screen.res][0] - w, sy, buf, eAlignCentre);
	...
	HUD_AABITMAP(Damage_gauges[2].first_frame, bx, by, eAlignCentre);		
}
...
void hud_show_text_flash_icon(char *txt, int y, int bright)
{
	...
	// draw the box	
	const int x = (int) (( (float) (gr_screen.max_w - w)) / 2.0f - 1.0f);
	gr_rect(GetHUDCoordsCentred(x), y - 1, w + 2, h + 1);

	// string
	hud_set_gauge_color(HUD_TEXT_FLASH, HUD_C_BRIGHT);	
	HUD_STRING(0x8000, y, txt, eAlignCentre);
...
void hud_show_kills_gauge()
{
	if ( Kills_gauge.first_frame < 0 || Cmdline_splitscreen) {
		return;
	}
	...
	HUD_STRING(Kills_text_coords[gr_screen.res][0], Kills_text_coords[gr_screen.res][1], XSTR( "kills:", 223), eAlignRight);
	...
	HUD_STRING(Kills_text_val_coords[gr_screen.res][0]-w, Kills_text_val_coords[gr_screen.res][1], num_kills_string, eAlignRight);
}
...
void hud_support_view_blit()
{
	...
	HUD_AABITMAP(Support_view_gauge.first_frame, Support_view_coords[gr_screen.res][0], Support_view_coords[gr_screen.res][1], eAlignCentre);	
	HUD_STRING(Support_text_coords[gr_screen.res][0], Support_text_coords[gr_screen.res][1], XSTR( "support", 224), eAlignCentre);

	if ( Hud_support_view_fade > 1 ) {
		if ( !timestamp_elapsed(Hud_support_view_fade) ) {
			if ( Hud_support_view_abort){
				HUD_STRING(0x8000, Support_text_val_coords[gr_screen.res][1], XSTR( "aborted", 225), eAlignCentre);
			} else {
				HUD_STRING(0x8000, Support_text_val_coords[gr_screen.res][1], XSTR( "complete", 1407), eAlignCentre);
			}
			return;
		} else {
			Hud_support_view_abort = 0;
	...
	if ( Player_ai->ai_flags & AIF_BEING_REPAIRED ) {
		...
		HUD_STRING(0x8000, Support_text_val_coords[gr_screen.res][1], outstr, eAlignCentre);
		...
		if ( Hud_support_objnum == -1 ) {
			...
			if (!show_time) {
				HUD_STRING(Support_text_dock_coords[gr_screen.res][0], Support_text_val_coords[gr_screen.res][1], outstr, eAlignCentre);
			} else {			
				HUD_STRING(Support_text_dock_coords[gr_screen.res][0], Support_text_val_coords[gr_screen.res][1], outstr, eAlignCentre);
			}
		}
...
bool hud_gauge_active(int gauge_index)
{
	if(Cmdline_splitscreen)
	{
		switch(gauge_index)
		{
		case HUD_TARGET_SHIELD_ICON: // Takes up too much space on the left
		case HUD_TALKING_HEAD: // Not working in splitscreen
		case HUD_AUTO_TARGET: // Takes up too much space on the right
		case HUD_AUTO_SPEED: // Takes up too much space on the right

		case HUD_OBJECTIVES_NOTIFY_GAUGE: // Not working
		case HUD_ETS_GAUGE: // No input assigned
			return false;
		}
	}
	...
	return hud_config_show_flag_is_set(gauge_index) != 0;
}
...
void hud_maybe_display_subspace_notify()
{
	...
	HUD_STRING(0x8000, Subspace_text_coords[gr_screen.res][1],XSTR( "subspace drive", 233), eAlignCentre);
	if ( warp_aborted ) {
		HUD_STRING(0x8000, Subspace_text_val_coords[gr_screen.res][1],XSTR( "aborted", 225), eAlignCentre);
	} else {
		HUD_STRING(0x8000, Subspace_text_val_coords[gr_screen.res][1],XSTR( "engaged", 234), eAlignCentre);
	}
}
...
void hud_maybe_display_red_alert()
{
	...
	HUD_STRING(0x8000, Red_text_coords[gr_screen.res][1], XSTR( "downloading new", 235), eAlignLeft);
	HUD_STRING(0x8000, Red_text_val_coords[gr_screen.res][1], XSTR( "orders...", 236), eAlignLeft);
...
void hud_maybe_display_objective_message()
{
	...
	// blit the background
	hud_set_gauge_color(HUD_OBJECTIVES_NOTIFY_GAUGE);
	HUD_AABITMAP(Objective_display_gauge.first_frame, Objective_display_coords[gr_screen.res][0], Objective_display_coords[gr_screen.res][1], eAlignLeft);	
	...
	case PRIMARY_GOAL:
		HUD_STRING(0x8000, Objective_text_coords[gr_screen.res][1],XSTR( "primary objective", 237));
		break;
	case SECONDARY_GOAL:
		HUD_STRING(0x8000, Objective_text_coords[gr_screen.res][1],XSTR( "secondary objective", 238));
		break;
	case BONUS_GOAL:
		HUD_STRING(0x8000, Objective_text_coords[gr_screen.res][1],XSTR( "bonus objective", 239));
		break;
	}
	...
	case SECONDARY_GOAL:
		...
		case GOAL_FAILED:
			...
			HUD_STRING(0x8000, Objective_text_val_coords[gr_screen.res][1], buf);
			break;
		default:
	...
	case BONUS_GOAL:
		switch(Objective_display.goal_status) {
		case GOAL_FAILED:
			HUD_STRING(0x8000, Objective_text_val_coords[gr_screen.res][1], XSTR( "failed", 242));
			break;
		default:
			HUD_STRING(0x8000, Objective_text_val_coords[gr_screen.res][1], XSTR( "complete", 226));
			break;
		}		
		break;
	}
...
void HUD_reset_clip()
{
	...
	gr_reset_clip_splitscreen_safe(hx, hy);
}
...
const int HUD_WIDTH = 1024;

int GetHUDCoordsCentred(const int x)
{
	if(Cmdline_splitscreen)
	{
		int offsetFromCentre = x - (HUD_WIDTH / 2);
		return (HUD_WIDTH / 4) + offsetFromCentre;
	}
...
int GetHUDCoordsRight(const int x)
{
	if(Cmdline_splitscreen)
	{
		const int offsetFromRight = HUD_WIDTH - x;
		return HUD_WIDTH / 2 - offsetFromRight;
	}

	return x;
}

void HUD_AABITMAP(const int lBitmap, const int x, const int y, const eHudAlign lAlignment)
{
	if(lAlignment == eAlignCentre)
	{
		GR_AABITMAP(lBitmap, GetHUDCoordsCentred(x), y);
	}
	else if(lAlignment == eAlignRight)
	{
		GR_AABITMAP(lBitmap, GetHUDCoordsRight(x), y);
	}
	else
	{
		GR_AABITMAP(lBitmap, GetHUDCoordsLeft(x), y);
	}
}

void HUD_AABITMAP_EX(
	const int lBitmap, 
	...
	const int sx,
	const int sy,
	const eHudAlign lAlignment)
{
	if(lAlignment == eAlignCentre)
	{
		GR_AABITMAP_EX(lBitmap, 
			GetHUDCoordsCentred(x), y, 
			w, h, sx, sy);
	}
	else if(lAlignment == eAlignRight)
	{
		GR_AABITMAP_EX(lBitmap, 
			GetHUDCoordsRight(x), y, 
			w, h, sx, sy);
	}
	else
	{
		GR_AABITMAP_EX(lBitmap,
			GetHUDCoordsLeft(x), y, 
			w, h, sx, sy);
	}
}

void HUD_STRING(const int x, const int y, const char * const lpString, const eHudAlign lAlignment)
{
	if(x == 0x8000)
	{
		gr_string(x, y, lpString);
	}
	else if(lAlignment == eAlignCentre)
	{
		gr_string(GetHUDCoordsCentred(x), y, lpString);
	}
	else if(lAlignment == eAlignRight)
	{
		gr_string(GetHUDCoordsRight(x), y, lpString);
	}
	else
	{
		gr_string(GetHUDCoordsLeft(x), y, lpString);
	}
}
...
void HUD_PRINTF( int x, int y, const eHudAlign lAlignment, char * format, ... )
{
	if(x != 0x8000)
	{
		if(lAlignment == eAlignCentre)
		{
			x = GetHUDCoordsCentred(x);
		}
		else if(lAlignment == eAlignRight)
		{
			x = GetHUDCoordsRight(x);
		}
		else
		{
			x = GetHUDCoordsLeft(x);
		}
	}
