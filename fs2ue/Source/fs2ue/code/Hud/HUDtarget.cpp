...
void hud_target_hotkey_clear( int k )
{
	...
	if ( Player->current_hotkey_set == k )		// clear this variable if we removed the bindings
...
void hud_keyed_targets_clear()
{
	...
	for (i = 0; i < MAX_KEYED_TARGETS; i++ )
		list_init( &(Player->keyed_targets[i]) );
	Player->current_hotkey_set = -1;
...
void hud_tri(float x1,float y1,float x2,float y2,float x3,float y3)
{
	...
	verts[0].sx = GetHUDCoordsLeft(x1);	verts[0].sy = y1;
	verts[1].sx = GetHUDCoordsLeft(x2);	verts[1].sy = y2;
	verts[2].sx = GetHUDCoordsLeft(x3);	verts[2].sy = y3;

	uint saved_mode = gr_zbuffer_get();
	
	gr_zbuffer_set( GR_ZBUFF_NONE );
	
	gr_tmapper( 3, vertlist, TMAP_FLAG_NO_TEXTURE );
...
{
	gr_line(fl2i(GetHUDCoordsLeft(x1)),fl2i(y1),fl2i(GetHUDCoordsLeft(x2)),fl2i(y2));
	gr_line(fl2i(GetHUDCoordsLeft(x2)),fl2i(y2),fl2i(GetHUDCoordsLeft(x3)),fl2i(y3));
	gr_line(fl2i(GetHUDCoordsLeft(x3)),fl2i(y3),fl2i(GetHUDCoordsLeft(x1)),fl2i(y1));
}
...
void hud_show_selection_set()
{
	...
	set = Player->current_hotkey_set;
	if ( set == -1 )
		return;

	Assert ( (set >= 0) && (set < MAX_KEYED_TARGETS) );
	plist = &(Player->keyed_targets[set]);
...
void hud_show_targeting_gauges(float frametime, int in_cockpit)
{
	...
	Player->lead_indicator_active = 0;
...
void hud_show_lead_indicator(vector *target_world_pos)
{
	...
	Player->lead_indicator_active = 1;

	// test if the target is moving at all
	if ( vm_vec_mag_quick(&targetp->phys_info.vel) < 0.1f)		// Find distance!
		Player->lead_target_pos =  *target_world_pos;
	else {
		vm_vec_normalize(&target_moving_direction);
		vm_vec_scale(&target_moving_direction,target_moved_dist);
		vm_vec_add(&Player->lead_target_pos, target_world_pos, &target_moving_direction );
		polish_predicted_target_pos(target_world_pos, &Player->lead_target_pos, dist_to_target, &last_delta_vector, 1); // Not used:, float time_to_enemy)
	}

	g3_rotate_vertex(&lead_target_vertex,&Player->lead_target_pos);
...
//			player_match_target_speed("", "", XSTR("Matching speed of newly acquired target",-1));
			...
			if ( Player->flags & PLAYER_FLAGS_MATCH_TARGET )
				Player->flags &= ~PLAYER_FLAGS_MATCH_TARGET;		// no more target matching.
		}
...
void hud_draw_offscreen_indicator(vertex* target_point, vector *tpos, float distance)
{
	...
	if (on_right) {
		...
		if ( buf[0] ) {
			HUD_STRING( fl2i(xpos - w - 10), fl2i(ypos - h/2.0f+0.5f), buf, eAlignLeft);
		}
	}
	else if (on_left) {
		x1 = x4 = (xpos-1);
		...
		if ( buf[0] ) {
			HUD_STRING(fl2i(xpos + 10), fl2i(ypos - h/2.0f+0.5f), buf, eAlignLeft);
		}
	}
	else if (on_top) {
		y1 = y4 = (ypos-1);
		...
		if ( buf[0] ) {
			HUD_STRING(fl2i(xpos - w/2.0f+0.5f), fl2i(ypos+10), buf, eAlignLeft);
		}
	}
	else if (on_bottom) {
		y1 = y4 = (ypos+2);
		...
		if ( buf[0] ) {
			HUD_STRING(fl2i(xpos - w/2.0f+0.5f), fl2i(ypos-h-10), buf, eAlignLeft);
		}
	}
...
void hud_show_afterburner_gauge()
{
	...
	if ( clip_h > 0) {
		HUD_AABITMAP_EX(Energy_bar_gauges.first_frame, 
			Aburn_coords[gr_screen.res][0], 
			Aburn_coords[gr_screen.res][1],
			w,clip_h,0,0, eAlignCentre);		
	}

	if ( clip_h <= Aburn_coords[gr_screen.res][3] ) {		
		HUD_AABITMAP_EX(Energy_bar_gauges.first_frame+1, 
			Aburn_coords[gr_screen.res][0], Aburn_coords[gr_screen.res][1]+clip_h,
			w,h-clip_h,0,clip_h, eAlignCentre);
	} 	
}
...
void hud_show_weapon_energy_gauge()
{
	...
	if ( percent_left <= 0.3 ) {
		...
		hud_num_make_mono(buf);
		HUD_STRING(Weapon_energy_text_coords[gr_screen.res][0], Weapon_energy_text_coords[gr_screen.res][1], buf, eAlignCentre);
	}
	...
	if ( clip_h > 0 ) {
		GR_AABITMAP_EX(Energy_bar_gauges.first_frame+2, 
			GetHUDCoordsCentred(Wenergy_coords[gr_screen.res][0]), Wenergy_coords[gr_screen.res][1], w,clip_h,0,0);		
	}

	if ( clip_h <= Wenergy_coords[gr_screen.res][3] ) {
		GR_AABITMAP_EX(Energy_bar_gauges.first_frame+3, 
			GetHUDCoordsCentred(Wenergy_coords[gr_screen.res][0]), Wenergy_coords[gr_screen.res][1] + clip_h, w,h-clip_h,0,clip_h);		
	}
...
void hud_show_secondary_weapon(int count, ship_weapon *sw, int dual_fire)
{
	...
	for ( i = 0; i < count; i++ ) {
		...
		if ( sw->current_secondary_bank == i ) {
			emp_hud_printf(Weapon_sunlinked_x[gr_screen.res], Weapon_secondary_y[gr_screen.res][i] - np*12, EG_NULL, eAlignLeft, "%c", Lcl_special_chars + 2);			

			if ( dual_fire ) {
				emp_hud_printf(Weapon_slinked_x[gr_screen.res], Weapon_secondary_y[gr_screen.res][i] - np*12, EG_NULL, eAlignLeft, "%c", Lcl_special_chars + 2);				
			}

			emp_hud_string(Weapon_secondary_name_x[gr_screen.res], Weapon_secondary_y[gr_screen.res][i] - np*12, i ? EG_WEAPON_S1 : EG_WEAPON_S2, weapon_name, eAlignRight);
			...
			if ( (sw->secondary_bank_ammo[i] > 0) && (sw->current_secondary_bank >= 0) ) {
				...
				if ( (ms_till_fire >= 500) && ((wip->fire_wait >= 1 ) || (ms_till_fire > wip->fire_wait*1000)) ) {
					emp_hud_printf(Weapon_secondary_reload_x[gr_screen.res], Weapon_secondary_y[gr_screen.res][i] - np*12, EG_NULL, eAlignLeft, "%d", fl2i(ms_till_fire/1000.0f +0.5f));					
				}
			}
		} else {
			emp_hud_string(Weapon_secondary_name_x[gr_screen.res], Weapon_secondary_y[gr_screen.res][i] - np*12, i ? EG_WEAPON_S1 : EG_WEAPON_S2, weapon_name, eAlignRight);			
		...
		emp_hud_string(Weapon_secondary_ammo_x[gr_screen.res] - w, Weapon_secondary_y[gr_screen.res][i] - np*12, EG_NULL, ammo_str, eAlignRight);		
...
void hud_show_cmeasure_gague()
{
	...
	// blit the background
	HUD_AABITMAP(Cmeasure_gauge.first_frame, Cm_coords[gr_screen.res][0], Cm_coords[gr_screen.res][1], eAlignRight);	

	// blit text
	HUD_STRING(Cm_text_coords[gr_screen.res][0], Cm_text_coords[gr_screen.res][1], XSTR( "cm.", 327), eAlignRight);
	if ( !Player_ship ) {
		Int3();	// player ship doesn't exist?
		return;
	}
	HUD_PRINTF(Cm_text_val_coords[gr_screen.res][0], Cm_text_val_coords[gr_screen.res][1], eAlignRight, NOX("%02d"),Player_ship->cmeasure_count);
}
...
void hud_show_weapons()
{
	...
	lNumPrimary = sw->num_primary_banks;
	lNumSecondary = sw->num_secondary_banks;

	hud_set_gauge_color(HUD_WEAPONS_GAUGE);

	// draw top of primary display
	HUD_AABITMAP(Weapon_gauges[0].first_frame, Weapon_gauge_primary_coords[gr_screen.res][0][0], Weapon_gauge_primary_coords[gr_screen.res][0][1], eAlignRight);	
	
	emp_hud_string(Weapon_title_coords[gr_screen.res][0], Weapon_title_coords[gr_screen.res][1], EG_WEAPON_TITLE, XSTR( "weapons", 328), eAlignRight);		

	switch ( lNumPrimary ) {
	case 0:		
		// draw bottom of border		
		HUD_AABITMAP(Weapon_gauges[2].first_frame, Weapon_gauge_primary_coords[gr_screen.res][1][0], Weapon_gauge_primary_coords[gr_screen.res][1][1], eAlignRight);		

		emp_hud_string(Weapon_pname_coords[gr_screen.res][0][0], Weapon_pname_coords[gr_screen.res][0][1], EG_WEAPON_P1, XSTR( "<none>", 329), eAlignRight);		
	...
	case 1:
		...
		emp_hud_printf(Weapon_plink_coords[gr_screen.res][0][0], Weapon_plink_coords[gr_screen.res][0][1], EG_NULL, eAlignRight, "%c", Lcl_special_chars + 2);
		emp_hud_printf(Weapon_pname_coords[gr_screen.res][0][0], Weapon_pname_coords[gr_screen.res][0][1], EG_WEAPON_P2, eAlignRight, "%s", name);					
		break;

	case 2:
		// draw border to accomodate second primary weapon
		HUD_AABITMAP(Weapon_gauges[1].first_frame, Weapon_gauge_primary_coords[gr_screen.res][1][0], Weapon_gauge_primary_coords[gr_screen.res][1][1], eAlignRight);		

		// draw bottom of border
		HUD_AABITMAP(Weapon_gauges[2].first_frame, Weapon_gauge_primary_coords[gr_screen.res][2][0], Weapon_gauge_primary_coords[gr_screen.res][2][1], eAlignRight);
		...
		emp_hud_printf(Weapon_pname_coords[gr_screen.res][0][0], Weapon_pname_coords[gr_screen.res][0][1], EG_WEAPON_P1, eAlignRight, "%s", name);			
		...
		if ( sw->current_primary_bank == 1 || (Player_ship->flags & SF_PRIMARY_LINKED) ) {
			emp_hud_printf(Weapon_plink_coords[gr_screen.res][1][0], Weapon_plink_coords[gr_screen.res][1][1], EG_NULL, eAlignRight, "%c", Lcl_special_chars + 2);
		}
		emp_hud_printf(Weapon_pname_coords[gr_screen.res][1][0], Weapon_pname_coords[gr_screen.res][1][1], EG_WEAPON_P2, eAlignRight, "%s", name);		
		lNumPrimary = 0;
		break;
	...
	switch ( lNumSecondary ) {
	case 0:
		// draw the bottom of the secondary weapons
		HUD_AABITMAP(Weapon_gauges[4].first_frame, Weapon_gauge_secondary_coords[gr_screen.res][0][0], Weapon_gauge_secondary_coords[gr_screen.res][0][1] - 12*lNumPrimary - 1, eAlignRight);		

		emp_hud_string(Weapon_pname_coords[gr_screen.res][0][0], Weapon_secondary_y[gr_screen.res][0] - lNumPrimary*12, EG_WEAPON_S1, XSTR( "<none>", 329), eAlignRight);		
		break;

	case 1:
		// draw the bottom of the secondary weapons
		HUD_AABITMAP(Weapon_gauges[4].first_frame, Weapon_gauge_secondary_coords[gr_screen.res][1][0], Weapon_gauge_secondary_coords[gr_screen.res][1][1] - 12*lNumPrimary - 1, eAlignRight);		
	...
	case 2:
		...
		HUD_AABITMAP(Weapon_gauges[3].first_frame, Weapon_gauge_secondary_coords[gr_screen.res][2][0], Weapon_gauge_secondary_coords[gr_screen.res][2][1] - lNumPrimary*12, eAlignRight);		

		// draw the bottom of the secondary weapons
		HUD_AABITMAP(Weapon_gauges[4].first_frame, Weapon_gauge_secondary_coords[gr_screen.res][3][0], Weapon_gauge_secondary_coords[gr_screen.res][3][1] - 12*lNumPrimary, eAlignRight);		
	...
	case 3:
		...
		HUD_AABITMAP(Weapon_gauges[3].first_frame, Weapon_gauge_secondary_coords[gr_screen.res][3][0], Weapon_gauge_secondary_coords[gr_screen.res][3][1] - lNumPrimary*12, eAlignRight);		

		// draw the bottom of the secondary weapons
		HUD_AABITMAP(Weapon_gauges[4].first_frame, Weapon_gauge_secondary_coords[gr_screen.res][4][0], Weapon_gauge_secondary_coords[gr_screen.res][4][1] - 12*lNumPrimary, eAlignRight);		
...
void hud_auto_target_icon()
{
	...
	HUD_AABITMAP(Toggle_gauge.first_frame+frame_offset, Toggle_target_gauge_coords[gr_screen.res][0], Toggle_target_gauge_coords[gr_screen.res][1], eAlignRight);	
	...
	HUD_STRING(Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_AUTOT][0], Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_AUTOT][1], XSTR("auto", 1463), eAlignRight);
	HUD_STRING(Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_TARGET][0], Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_TARGET][1], XSTR("target", 1465), eAlignRight);
}
...
void hud_auto_speed_match_icon()
{
	...
	HUD_AABITMAP(Toggle_gauge.first_frame+frame_offset, Toggle_speed_gauge_coords[gr_screen.res][0], Toggle_speed_gauge_coords[gr_screen.res][1], eAlignRight);	
	...
	HUD_STRING(Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_AUTOS][0], Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_AUTOS][1], XSTR("auto", 1463), eAlignRight);
	HUD_STRING(Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_SPEED][0], Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_SPEED][1], XSTR("speed", 1464), eAlignRight);
}
