...
const int HUDSHIELD_HIT_DURATION = 1400;	// time a shield quadrant flashes after being hit
...
void hud_escort_show_icon(int index, object *objp)
{
	...
	emp_hud_printf( Escort_gauge_text_coords[gr_screen.res][index][2][0]+offset, Escort_gauge_text_coords[gr_screen.res][index][2][1], EG_NULL, eAlignRight, "%d", screen_integrity);	
}
...
void hud_escort_show_icon_dogfight(int index)
{
	...
	// show ship integrity
	if(objp == NULL){	
		emp_hud_printf( Escort_gauge_text_coords[gr_screen.res][index][2][0] - stat_shift, Escort_gauge_text_coords[gr_screen.res][index][2][1], EG_NULL, eAlignLeft, "%d", Net_players[np_index].player->stats.m_kill_count_ok);	
	} else {
		emp_hud_printf( Escort_gauge_text_coords[gr_screen.res][index][2][0] - stat_shift, Escort_gauge_text_coords[gr_screen.res][index][2][1], EG_NULL, eAlignLeft, "(%d%%) %d", hull_integrity, Net_players[np_index].player->stats.m_kill_count_ok);	
	}
}
...
void hud_escort_ship_hit(object *objp, int quadrant)
{
	...
	for ( i = 0; i < Num_escort_ships; i++ ) {
		if ( Escort_ships[i].objnum == OBJ_INDEX(objp) ) {
			...
			hud_gauge_popup_start(HUD_ESCORT_VIEW);
			if ( quadrant >= 0 ) {
				shi->shield_hit_timers[num] = timestamp(HUDSHIELD_HIT_DURATION);
			} else {
				shi->shield_hit_timers[HULL_HIT_OFFSET] = timestamp(HUDSHIELD_HIT_DURATION);
			}
		}
	}
}
