...
void asteroid_render(object * obj)
{
	if (Asteroids_enabled) {
		...
		Assert((obj->instance >= 0) && (obj->instance < MAX_ASTEROIDS));
		asteroid *asp = &Asteroids[obj->instance];
...
void asc_get_relvec(vector *relvec, object *other_obj, vector *hitpos)
{
	...
	while (lTryCount--) 
	{
		...
		const float mag = vm_vec_mag_quick(relvec);
...
float asteroid_get_fireball_scale_multiplier(int num)
{
	...
	{
		if(Asteroids[num].type == ASTEROID_TYPE_BIG)
		{
			return 1.5f;
		}
	}
