	...
	*/
	...
	while(1)	{
		...
		if ( Sel_mode==1 ) {
			...
			if (mouse_down_count(MouseController::MOUSE_RIGHT_BUTTON)) {
				Orig_pos_x = Mouse_x;
				Orig_pos_y = Mouse_y;
				for (i=0;i<num_pts;i++) {
					Selected[i] = FALSE;
			...
			if (mouse_down(MouseController::MOUSE_RIGHT_BUTTON)) {
				Draw_sel_box = TRUE;
				End_pos_x = Mouse_x;
				End_pos_y = Mouse_y;
			}
			if (mouse_up_count(MouseController::MOUSE_RIGHT_BUTTON)) {
				Draw_sel_box = FALSE;
				End_pos_x = Mouse_x;
				End_pos_y = Mouse_y;
