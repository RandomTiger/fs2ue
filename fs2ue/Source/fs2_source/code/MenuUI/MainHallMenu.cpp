...
*/
	...
	g_MouseController.SetPos(Main_hall->door_anim_coords[READY_ROOM_REGION][2],Main_hall->door_anim_coords[READY_ROOM_REGION][3]);	

	Main_hall_inited = 1;

	// determine if we have a right click
	Main_hall_right_click = g_MouseController.IsDown(MouseController::MOUSE_RIGHT_BUTTON);
	...
	if(Cmdline_voicer == 1)
	{
		...
		{
			//g_TextToSpeech.Play("Voice Recognition Initialized");
		}
		else
		{
			const int bufferSize = 1024;
			char buffer[bufferSize];
			sprintf_s(buffer, bufferSize, "Voice Recognition is NOT Initialized. Error number %s",g_VoiceRecognition.GetLastErrorString());
...
void main_hall_handle_right_clicks()
{
	...
	if(!Main_hall_right_click){
		if(g_MouseController.IsDown(MouseController::MOUSE_RIGHT_BUTTON)){
			...
			g_MouseController.SetPos(Main_hall->door_anim_coords[new_region][2],Main_hall->door_anim_coords[new_region][3]);			
	...
	else if(Main_hall_right_click && !(g_MouseController.IsDown(MouseController::MOUSE_RIGHT_BUTTON))){
