...
// C6262	Excessive stack usage	Function uses '17140' bytes of stack:  exceeds /analyze:stacksize '16384'.  Consider moving some data to heap.	
void post_process_mission()
{
	int			i;
	int			indices[MAX_SHIPS];
	p_object		*p_objp;
	ship_weapon	*swp;
	ship_obj *so;

	// the player_start_shipname had better exist at this point!
	int lPlayer_start_shipnum = ship_name_lookup( Player_start_shipname );
	Assert ( lPlayer_start_shipnum != -1 );
	Assert ( Player_start_pobject.flags & P_SF_PLAYER_START_VALID );

	// Assign objnum, shipnum, etc. to the player structure
	const int objnum = Ships[lPlayer_start_shipnum].objnum;

	FlushAllAllocatedBeahviours();
	ClearPlayerList();
	SetupPlayerShip(objnum, Player);

	if(Cmdline_splitscreen)
	{
		vector lNewPos = Player_obj->pos;

		for(int i = 1; i < kiNumPlayerSplitscreen; i++)
		{
			int liObjNum = -1;
			const int kiBuffSize = 64;
			char lacBuffer[kiBuffSize];
			sprintf_s(lacBuffer, kiBuffSize, "Alpha %d", i + 1);

			const int lExtra_start_shipnum = ship_name_lookup(lacBuffer);
			if(lExtra_start_shipnum != -1)
			{
				// Theres a ship with that name setup in the level
				liObjNum = Ships[lExtra_start_shipnum].objnum;			
			}
			else
			{
				// Create our own on the fly
				lNewPos.x += 50.0f;
				liObjNum = ship_create(&vmd_identity_matrix, &lNewPos, Player_start_pobject.ship_class);
			}

			SetupPlayerShip(liObjNum, PlayerCoOp);
		}
	}
