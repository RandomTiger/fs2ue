...
void gr_opengl_bitmap_ex(int x,int y,int w,int h,int sx,int sy)
{
	...
	sptr = (ubyte *)( bmp->GetDataAsUint() + (sy*bmp->w+sx) );
...
int gr_opengl_tcache_set(int bitmap_id, int bitmap_type, float *u_ratio, float *v_ratio, int fail_on_full = 0, const bool justCacheNoSet = false)
{
	return 1;
}
...
void gr_opengl_init()
{
	...
	gr_screen.gf_set_bitmap_ptr = gr_opengl_set_bitmap;
	...
	gr_screen.gf_set_palette_ptr = gr_opengl_set_palette;
	...
	gr_screen.gf_fog_set_ptr = gr_opengl_fog_set;	
	...
	gr_screen.gf_tcache_set_ptr = gr_opengl_tcache_set;
