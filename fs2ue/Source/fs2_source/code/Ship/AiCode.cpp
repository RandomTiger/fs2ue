...
int ai_formation_object_get_slotnum(int objnum, object *objp)
{
	...
	object *o = GET_FIRST(&obj_used_list);
	for ( ; o != END_OF_LIST(&obj_used_list); o = GET_NEXT(o) ) {
		if (objp == o)
			break;
...
float dock_orient_and_approach(object *objp, object *dobjp, int dock_mode)
{
	...
	float			fdist = AIUNINITIALIZED_VALUE;
...
//		else
	...
	// -- Note, A lot of callers don't care about fdist, so OK to return ERROR value: Assert(fdist != AIUNINITIALIZED_VALUE);
	return fdist;
...
void ai_big_guard()
{
	...
	case AIS_GUARD_PATROL:
		...
		int time_choose = int(floor(log(length * 0.001) / log(2.0f)));
...
void ai_dock()
{
	...
	case AIS_DOCK_2: {
		...
		if ((r = maybe_dock_obstructed(Pl_objp, goal_objp,0)) != -1) {
			...
			Assert(dist != AIUNINITIALIZED_VALUE);
			...
			if (dist < 2*flFrametime * (1.0f + fl_sqrt(goal_objp->phys_info.speed))) {
				...
				if (aip->submode == AIS_DOCK_3) {
					...
					if ((Pl_objp == Player_obj) || (goal_objp == Player_obj))
						g_ForceFeedback.Docked();  // shake player's joystick a little
				}
