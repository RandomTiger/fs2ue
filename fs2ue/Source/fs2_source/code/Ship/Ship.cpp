...
int parse_ship()
{
	...
	// C6262	Excessive stack usage	Function uses '43768' bytes of stack:  exceeds /analyze:stacksize '16384'.  
	...
	if ( sp->flags & SF_DYING )	{
		...
		if (sp->flags & SF_VAPORIZE) {
			...
			if (timestamp_elapsed(sp->final_death_time)) {
				...
				if (Cmdline_splitscreen == false && objp == Player_obj) {				
...
int ship_fire_primary(object * obj, int stream_weapons, int force)
{
		...
		if ( po->n_guns > 0 ) {
			...
			for ( j = 0; j < num_slots; j++ ) {
				...
				if((winfo_p->wi_flags & WIF_SHUDDER) && (obj == Player_obj) && !(Game_mode & GM_STANDALONE_SERVER)){
					...
					MsgShudderApply lMsg(500, (winfo_p->mass * winfo_p->max_speed) / 10.0f);
	...
	//				snd_play( &Snds[winfo_p->launch_snd] );
					...
					if (swp->current_primary_bank >= 0) {
						...
						g_ForceFeedback.PlayPrimaryShoot((int) ((wip->armor_factor + wip->shield_factor * 0.2f) * (wip->damage * wip->damage - 7.5f) * 0.45f + 0.6f) * 10 + 2000);
					}
				}
			}
			else {
		...
		if ( Weapon_info[weapon].launch_snd != -1 ) {
			...
			if (swp->current_secondary_bank >= 0) {
				...
				if (Player_ship->flags & SF_SECONDARY_DUAL_FIRE){
					g_ForceFeedback.PlaySecondaryShoot((int) (wip->cargo_size * 2.0f));
				} else {
					g_ForceFeedback.PlaySecondaryShoot((int) wip->cargo_size);
				}
			}
		}
...
int ship_set_primary(object *objp, int type)
{
	ship	*shipp;
	ship_weapon *swp;
	...
	swp = &shipp->weapons;
/*
	if ( swp->num_primary_banks < weaponIndex) {
		if ( objp == Player_obj ) {
...
*/
	...
	{
		shipp->flags &= ~SF_PRIMARY_LINKED;
		if(type == CHOOSE_PRIMARY_1_OFF && swp->num_primary_banks > 1)
		{
			swp->current_primary_bank = 1;
		}
		else if(type == CHOOSE_PRIMARY_2_OFF)
		{
			swp->current_primary_bank = 0;
		}
	}
			...
			if ( swp->secondary_bank_ammo[i] < swp->secondary_bank_start_ammo[i] ) {
				...
				if ( timestamp_elapsed(swp->secondary_bank_rearm_time[i]) ) {
					...
					if ( aip->rearm_first_missile == TRUE ) {
						...
						if (objp == Player_obj)
							g_ForceFeedback.PlayReloadEffect();
...
void ship_update_artillery_lock()
{
	...
	//		extern void ssm_create(vector *target, vector *start, int ssm_index, ssm_firing_info *override);
			...
			ssm_create(&temp, (vector *) &Objects[so->objnum].pos, 0, NULL);				
