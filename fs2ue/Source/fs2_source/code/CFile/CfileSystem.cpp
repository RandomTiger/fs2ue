...
std::vector<char *> gCFileSystemLeakList;
...
cf_file *cf_get_file(int index)
{
	...
	return &CfileSystem::File_blocks[block]->files[offset];
}

// Create a new file and return a pointer to it.
cf_file *cf_create_file()
{
	int block = CfileSystem::Num_files / CF_NUM_FILES_PER_BLOCK;
	int offset = CfileSystem::Num_files % CF_NUM_FILES_PER_BLOCK;
	
	if ( CfileSystem::File_blocks[block] == NULL )	{
		CfileSystem::File_blocks[block] = (cf_file_block *)malloc( sizeof(cf_file_block) );
		Assert( CfileSystem::File_blocks[block] != NULL);
	}

	CfileSystem::Num_files++;
...
void cf_build_secondary_filelist(char *cdrom_dir)
{
	...
	for (i=0; i<CF_MAX_FILE_BLOCKS; i++ )	{
		CfileSystem::File_blocks[i] = NULL;
	}
...
void cf_free_secondary_filelist()
{
	...
	// Init the file blocks	
	for (i=0; i<CF_MAX_FILE_BLOCKS; i++ )	{
		if ( CfileSystem::File_blocks[i] )	{
			free( CfileSystem::File_blocks[i] );
			CfileSystem::File_blocks[i] = NULL;
		}
	}
	CfileSystem::Num_files = 0;
...
int cf_find_file_location( char *filespec, int pathtype, char *pack_filename, int *size, int *offset, bool localize )
{
	...
	// Search the pak files and CD-ROM.
		...
		for (i=0; i<CfileSystem::Num_files; i++ )	{
		...
		do {
			...
			if (!(find.attrib & _A_SUBDIR)) {
				if ( !Get_file_list_filter || (*Get_file_list_filter)(find.name) ) {
					...
					gCFileSystemLeakList.push_back(list[num_files]);
...
void cf_free_leaks()
{
	for(unsigned int i = 0; i < gCFileSystemLeakList.size(); i++)
	{
		free(gCFileSystemLeakList[i]);
	}

	gCFileSystemLeakList.clear();
}
