...
void hud_render_target_setup(vector *camera_eye, matrix *camera_orient, float zoom)
{
	...
	HUD_set_clip(
		GetHUDPlayerStartX() + Target_window_coords[gr_screen.res][0],
		Target_window_coords[gr_screen.res][1],
		Target_window_coords[gr_screen.res][2],
		Target_window_coords[gr_screen.res][3]);
...
void hud_render_target_jump_node(object *target_objp)
{
	...
	emp_hud_printf(Targetbox_coords[gr_screen.res][TBOX_DIST][0]+hx, Targetbox_coords[gr_screen.res][TBOX_DIST][1]+hy, EG_TBOX_DIST, eAlignLeft, outstr);	
}
...
void hud_render_target_asteroid(object *target_objp)
{
	...
	emp_hud_printf(Targetbox_coords[gr_screen.res][TBOX_NAME][0], Targetbox_coords[gr_screen.res][TBOX_NAME][1], EG_TBOX_NAME, eAlignLeft, hud_name);	
	
	time_to_impact = asteroid_time_to_impact(target_objp);
	if ( time_to_impact >= 0 ) {
		emp_hud_printf(Targetbox_coords[gr_screen.res][TBOX_CLASS][0], Targetbox_coords[gr_screen.res][TBOX_CLASS][1], EG_TBOX_CLASS, eAlignLeft, NOX("impact: %.1f sec"), time_to_impact);	
	}
#endif
}
...
void hud_render_target_ship_info(object *target_objp)
{
	...
	emp_hud_printf(Targetbox_coords[gr_screen.res][TBOX_CLASS][0], Targetbox_coords[gr_screen.res][TBOX_CLASS][1], EG_TBOX_CLASS, eAlignLeft, temp_name);
	...
	emp_hud_printf(Targetbox_coords[gr_screen.res][TBOX_HULL][0]-w, Targetbox_coords[gr_screen.res][TBOX_HULL][1], EG_TBOX_HULL, eAlignLeft, "%s", outstr);	
	hud_set_gauge_color(HUD_TARGET_MONITOR);
