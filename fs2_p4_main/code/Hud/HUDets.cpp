...
void hud_show_ets()
{
	...
	for ( j = 0; j < 3; j++ ) {
		...
		HUD_PRINTF(Ets_gauge_info[i].letter_coords[0], Ets_gauge_info[i].letter_coords[1], eAlignRight, NOX("%c"), Ets_gauge_info[j].letter); 
		i++;
	}
	...
	for ( j = 0; j < 3; j++ ) {
		...
		if ( index < NUM_ENERGY_LEVELS-1 ) {
			...
			HUD_AABITMAP_EX(Ets_gauge.first_frame,x,y,w,clip_h,0,0, eAlignRight);			
			...
			HUD_AABITMAP_EX(Ets_gauge.first_frame, x, y_start, w, y_end-y_start, 0, ETS_bar_h[gr_screen.res]-clip_h, eAlignRight);			
		}
		...
		if ( index > 0 ) {
			...
			HUD_AABITMAP_EX(Ets_gauge.first_frame+1, x, y_start, w, y_end-y_start, 0, clip_h, eAlignRight);			
			...
			HUD_AABITMAP_EX(Ets_gauge.first_frame+2, x,y,w,ETS_bar_h[gr_screen.res]-clip_h,0,0, eAlignRight);			
		}
		i++;
	}
