	...
	if( optional_string("$BeamInfo:")){
		...
		while( optional_string("$Section:") ){
			...
			if(!Fred_running){
				...
				if(i.texture >= 0){
					bm_lock(i.texture, kDefaultBpp, BMP_TEX_OTHER);
					bm_unlock(i.texture);
				}
			}
	...
	switch (wip->render_type) {
		case WRT_LASER: {
			...
			if(wip->laser_glow_bitmap >= 0){
				...
				gr_set_bitmap(wip->laser_glow_bitmap, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL, weapon_glow_alpha_d3d);
...
void weapon_area_apply_blast(vector *force_apply_pos, object *ship_obj, vector *blast_pos, float blast, int make_shockwave)
{
	...
	if (make_shockwave) {
		...
		if (ship_obj == Player_obj) {
			g_ForceFeedback.PlayVectorEffect(&vec_blast_to_ship, blast * 2.0f);
		}
	} else {
