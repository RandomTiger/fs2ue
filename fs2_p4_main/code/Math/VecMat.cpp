...
const float SMALL_NUM = 1e-7f;
const float SMALLER_NUM = 1e-20f;
const float	CONVERT_RADIANS = 0.017453f;		// conversion factor from degrees to radians
...
float vm_vec_copy_normalize(vector *dest,vector *src)
{
	...
	if (m <= 0.0f || _finite(m) == false || _isnan(m)) {
