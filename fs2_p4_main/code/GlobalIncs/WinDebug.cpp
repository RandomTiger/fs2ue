...
{
	gbStrdupDupFreeTestEnbaled = true;
...
#endif
{
	...
	#ifndef NDEBUG
		dst = (char *)vm_malloc( len, filename, line );
	#else
		dst = (char *)vm_malloc( len );
	#endif
	strcpy_s( dst, len, ptr );

	gStrdupLeakList.push_back(dst);
	return dst;
}
...
void vm_strdup_free()
{
	...
	for(unsigned int i = 0; i < gStrdupLeakList.size(); i++)
	{
		if(gStrdupLeakList[i])
		{
			vm_free(gStrdupLeakList[i]);
			gStrdupLeakList[i] = 0;
		}
	}
}
...
#endif
{
	...
	if(gbStrdupDupFreeTestEnbaled)
	{
		for(unsigned int i = 0; i < gStrdupLeakList.size(); i++)
		{
			if(gStrdupLeakList[i] == ptr)
			{
				// This would end up being deleted twice
				gStrdupLeakList[i] = 0;
			}
		}
	}
